/*
 * <h2>Copyright</h2> Copyright (c) 1998-2011 Interworld Transport. All rights
 * reserved.<br>
 * ----------------------------------------------------------------
 * ---------------- <br> ---com.interworldtransport.cladosG.MonadRealD<br>
 * ------
 * --------------------------------------------------------------------------
...
public class MonadRealD extends MonadAbstract
{
	...
	public MonadRealD(MonadRealD pM)
	{
		...
		cM = new RealD[getAlgebra().getGProduct().getBladeCount()]; // length of
																	// cM is
																	// bladecount
	...
	public MonadRealD(String pMonadName, String pAlgebraName,
					...
					throws BadSignatureException, CladosMonadException
	{
		setAlgebra(new AlgebraRealD(pAlgebraName, new Foot(pFootName,
					...
					throws BadSignatureException, CladosMonadException
	{
		if (pC[0] != null)
		{
			...
			throw new CladosMonadException(this,
							"RealD cM[0] is to be set to null.  There could be more nulls too.");
		setName(pMonadName);
		setFrameName(pFrameName);
		//algebra.getFootPoint().appendIfUniqueRFrame(pFrameName);
		...
		if (pC.length == getAlgebra().getGProduct().getBladeCount())
			...
			throw new CladosMonadException(this,
							"RealD cM array size does not match bladecount appropriate for Signature.");
		setGradeKey();
	}
	...
	public MonadRealD dualLeft()
	{
		short[] tSpot = getAlgebra().getGProduct()
						.getGradeRange((short) (getAlgebra().getGProduct()
										.getGradeCount() - 1));
		RealD[] tNewCoeff = new RealD[getAlgebra().getGProduct()
	 ...
	 */
	@Override
	public MonadRealD gradeSupress(int pGrade)
	{
		if (pGrade >= 0
						& pGrade <= getAlgebra().getGProduct().getGradeCount() - 1)
		{
	...
	public void setCoeff(RealD[] ppC) throws CladosMonadException
	{
		if (ppC.length == getAlgebra().getGProduct().getBladeCount())
		{
			...
			throw new CladosMonadException(this,
							"RealD array passed in for coefficient copy is the wrong length");
	}
