...
public class AlgebraRealF extends AlgebraAbstract
{
	 ...
	 * It takes the RealF number type from the Foot and points at the offered GProduct causing
	 * it to be shared. It takes in one string for the algebra name as well and then produces 
	 * an AlgebraRealF. If anything is wrong with the signature it throws an exception. 
						...
						GProduct pGP)
	{
		...
		setGProduct(pGP);
		gBasis = pGP.getBasis();
		protoNumber = new RealF(pF.getCardinal(), 1.0f);
	}

	/**
	 * This is the constructor that assumes a Foot has been instantiated, so it takes
	 * the RealF number type from there. It takes in two strings (one name and a 
	 * product signature) and the Foot and produces an AlgebraRealF. If anything is wrong 
							...
							GeneratorRangeException
	{
		...
		protoNumber = new RealF(pFoot.getCardinal(), 1.0f);
	}
	 ...
	 * and the example RealF and produces an AlgebraRealF. If anything is wrong with
	 ...
	 * @param pSig			This is the signature of the GProduct
	 * @param pF			This is the number type to use expressed as a RealF
	 * @throws BadSignatureException
	 * This constructor creates a new GProduct which requires a signature for the generators.
	 * This signature string must be parse-able or this exception is thrown.
	 * @throws GeneratorRangeException
							...
							GeneratorRangeException
	{
		...
		setGProduct(new GProduct(pSig));
		gBasis = gProduct.getBasis();
		protoNumber = new RealF(foot.getCardinal(), 1.0f);
	}
}