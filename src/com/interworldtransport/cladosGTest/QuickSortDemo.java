...
/**
 * There is a speed improvement for sorting keys in an canonical basis
 ...
 * algorithm should avoid the overhead.
 * 
...
{ 
	...
	public long getMedian(int left, int right)
	{
		...
		if (vKey[left]>vKey[center]) swap(left, center);
		if (vKey[left]>vKey[right]) swap(left, right);
		if (vKey[center]>vKey[right]) swap(center, right);
		swap(center, right);
	 ...
	 * @return int	
	 * Remember that this sort is recursive. 
	 * I'm not going to document how it works here. 
	 * Go look it up.
	 */
