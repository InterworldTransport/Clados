...
public class MonadComplexF extends MonadAbstract {
	...
	public static boolean isScaledIdempotent(MonadComplexF pM) throws CladosMonadException, FieldException {
		...
		ComplexF fstnzeroC = copyOf(pM.getCoeff((short) 0)); // Grab copy of Scalar part
		...
		check1 = (new MonadComplexF(pM)).scale(fstnzeroC.invert()); // No risk of inverting a zero. Caught at top
	 ...
	 */
	public AlgebraComplexF algebra;
	/**
	 ...
	 */
	public MonadComplexF(String pName, MonadComplexF pM) throws BadSignatureException, CladosMonadException {
		setName(pName);
		setAlgebra(pM.getAlgebra());
		setFrameName(pM.getFrameName());
	...
	public MonadComplexF(String pMonadName, String pAlgebraName, String pFrameName, String pFootName, String pSig,
		...
		setAlgebra(new AlgebraComplexF(pAlgebraName, new Foot(pFootName, pC[0].getCardinal()), pSig, pC[0]));
	 ...
	 *                              of the wrong size.
	 */
	public MonadComplexF(String pMonadName, AlgebraComplexF pAlgebra, String pFrameName, ComplexF[] pC)
	...
	public MonadComplexF conjugate() {
		for (short k = 0; k < getAlgebra().getGradeCount(); k++)
			cM[k].conjugate();
		return this;
	}
	...
	public MonadComplexF dualLeft() {
		...
		ComplexF[] tNewCoeff = new ComplexF[getAlgebra().getBladeCount()];
	 ...
	 */
	public AlgebraComplexF getAlgebra() {
		return algebra;
	}
	 ...
	 * @return ComplexF
	 */
	public ComplexF getCoeff(short pB) {
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF multiplyAntisymm(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
		if (!isReferenceMatch(this, pM))
		...
		MonadComplexF halfTwo = new MonadComplexF(this);
		halfTwo.multiplyRight(pM);

		multiplyLeft(pM);
		subtract(halfTwo);

		scale(new ComplexF(cM[0], 0.5f, 0.0F));
		setGradeKey();
		return this;
	}
			...
			while (logKey >= 0) {
				...
				for (short i = tSpot[0]; i <= tSpot[1]; i++) {
					...
					// Looping through column blades in product array
					{
						...
						ComplexF tCtrbt = multiply(pM.getCoeff(i), cM[j]);
			...
			while (logKey >= 0.0D) {
				...
				for (short i = tSpot[0]; i <= tSpot[1]; i++) {
					...
					// Looping through column blades in product array
					{
						...
						// multiply the coefficients first
						ComplexF tCtrbt = multiply(cM[j], pM.getCoeff(i));
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF multiplySymm(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
		if (!isReferenceMatch(this, pM))
