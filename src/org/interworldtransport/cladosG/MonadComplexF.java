 ...
 * ------------------------------------------------------------------------ <br>
 * ---org.interworldtransport.cladosG.MonadComplexF<br>
 * -------------------------------------------------------------------- <p>
 ...
 */
public class MonadComplexF extends MonadAbstract {
	 ...
	 * 
	 * @param pM MonadComplexF This is the monad to be tested.
	 * 
	 * @return boolean
	 */
	public static boolean isGZero(MonadComplexF pM) {
	 ...
	 *                              checked.
	 */
	public static boolean isIdempotent(MonadComplexF pM) throws FieldBinaryException, CladosMonadException {
		if (isGZero(pM))
			return true;
		return ((MonadComplexF) CladosGMonad.COMPLEXF.copyOf(pM)).multiplyLeft(pM).isGEqual(pM);
	}
	 ...
	 */
	public static boolean isScaledIdempotent(MonadComplexF pM) throws CladosMonadException, FieldException {
		if (isIdempotent(pM))
			return true;
		...
		int k = 1;
		MonadComplexF check1 = (MonadComplexF) CladosGMonad.COMPLEXF.copyOf(pM);
		check1.multiplyLeft(pM); // We now have check1 = pM ^ 2
		if (isGZero(check1))
			return false; // pM is nilpotent at power=2
		ComplexF fstnzeroC = CladosFBuilder.copyOf(pM.getCoeff(0)); // Grab copy of Scalar part
		...
		check1 = ((MonadComplexF) CladosGMonad.COMPLEXF.copyOf(pM)).scale(fstnzeroC.invert());
	 ...
	 * 
	 * @return boolean
	 * @param pM     MonadComplexF The monad to be tested
	 ...
	 *                              checked.
	 */
	public static boolean isNilpotent(MonadComplexF pM, int pPower) throws FieldBinaryException, CladosMonadException {
		if (isGZero(pM))
			return true;

		MonadComplexF check1 = new MonadComplexF(pM);
		while (pPower>1) {
	 ...
	 * @return boolean
	 */
	public static boolean isReferenceMatch(MonadComplexF pM, MonadComplexF pN) {
	 ...
	 * 
	 * @param pM MonadComplexF This is the monad to be converted to XML.
	 * 
	 * @return String
	 */
	public static String toXMLString(MonadComplexF pM) {
	 ...
	 * @param pM MonadComplexF
	 */
	public MonadComplexF(MonadComplexF pM) {
		setName(pM.getName());
	 ...
	 */
	public MonadComplexF(String pName, MonadComplexF pM) throws CladosMonadException {
		setName(pName);
		setAlgebra(pM.getAlgebra());
		setFrameName(pM.getFrameName());
	 ...
	 * @param pSig         String
	 * @param pF           DivField Used to construct a ComplexF
	 * @throws BadSignatureException   This exception is thrown if the signature
	 *                                 string offered is rejected.
	 * @throws CladosMonadException    This exception is thrown if there is an issue
	...
	public MonadComplexF(String pMonadName, String pAlgebraName, String pFrameName, String pFootName, String pSig,
		...
		setAlgebra(CladosGAlgebra.COMPLEXF.create(pF, pAlgebraName, pFootName, pSig));//proto is ComplexF
		setName(pMonadName);
		setFrameName(pFrameName);

		cM = CladosFListBuilder.createComplexF(getAlgebra().getBladeCount());
		for (int k = 0; k < cM.length; k++)
			cM[k] = CladosFBuilder.createComplexF(getAlgebra().shareProtoNumber().getCardinal());
		setGradeKey();
	}
	 ...
	 * 
	 * @param pMonadName String
	 * @param pAlgebra   AlgebraComplexF
	 * @param pFrameName String
	 * @param pC         ComplexF[]
	 * @throws CladosMonadException This exception is thrown if there is an issue
	 *                              with the coefficients offered. The issues could
	 ...
	 *                              of the wrong size.
	 */
	public MonadComplexF(String pMonadName, Algebra pAlgebra, String pFrameName, ComplexF[] pC)
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF add(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
	 ...
	 */
	@Override
	public MonadComplexF conjugate() {
		IntStream.range(0, getAlgebra().getBladeCount()).parallel().forEach(k -> cM[k].conjugate());
		return this;
	}
	 ...
	 */
	@Override
	public MonadComplexF dualLeft() {
		int tSpot = getAlgebra().getGProduct().getBladeCount()-1; // tSpot points at the PScalar blade
		ComplexF[] tNewCoeff = new ComplexF[getAlgebra().getBladeCount()];// initialize results
	 ...
	 */
	@Override
	public MonadComplexF dualRight() {
		int tSpot = getAlgebra().getGProduct().getBladeCount()-1; // tSpot points at the PScalar blade
		ComplexF[] tNewCoeff = new ComplexF[getAlgebra().getBladeCount()];// initialize results
		IntStream.range(0, getAlgebra().getBladeCount()).forEach(j -> {
	 ...
	 */
	@Override
	public ComplexF getCoeff(int i) {
	 ...
	 * @return boolean
	 */
	public boolean isGEqual(MonadComplexF pM) {
		if (!isReferenceMatch(this, pM))
			return false;
	 ...
	 */
	@Override
	public ComplexF magnitude() throws CladosMonadException {
		try {
	 ...
	 * @throws CladosMonadException 
	 */
	public MonadComplexF multiplyAntisymm(MonadComplexF pM) throws FieldBinaryException, CladosMonadException {
		if (!isReferenceMatch(this, pM))
		...
		MonadComplexF halfTwo = ((MonadComplexF) CladosGMonad.COMPLEXF.copyOf(this)).multiplyRight(pM);
		multiplyLeft(pM).subtract(halfTwo).scale(ComplexF.newONE(cM[0].getCardinal()).scale(CladosConstant.BY2_F));
		setGradeKey();
		return this;
	}
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF multiplyLeft(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
		if (!isReferenceMatch(this, pM))
		...
		ComplexF[] tNewCoeff = CladosFListBuilder.createComplexFZERO(cM[0].getCardinal(), getAlgebra().getBladeCount());
					...
					if (!isZero(pM.getCoeff(i))) { // a weak sparse flag for pM
						for (int j = 0; j < getAlgebra().getBladeCount(); j++) { //loop on columns
							if (!isZero(cM[j])) { // a weak sparse flag
								ComplexF tCtrbt = multiply(pM.getCoeff(i), cM[j]); //coeffs first
			...
			for (int i = 0; i < getAlgebra().getBladeCount(); i++) { // Looping on rows
				if (!isZero(pM.getCoeff(i))) { // a weak sparse flag
					for (int j = 0; j < getAlgebra().getBladeCount(); j++){// Looping on columns
						ComplexF tCtrbt = multiply(pM.getCoeff(i), cM[j]); // coefficients first
						// find the blade to which this partial product contributes
						int prd = (Math.abs(getAlgebra().getGProduct().getResult(i, j)) - 1);
						// Adjust sign of contribution for product sign of blades
						tCtrbt.scale(Float.valueOf(getAlgebra().getGProduct().getSign(i, j)));
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF multiplyRight(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
	 ...
	 * @throws CladosMonadException 
	 */
	public MonadComplexF multiplySymm(MonadComplexF pM) throws FieldBinaryException, CladosMonadException {
		if (!isReferenceMatch(this, pM))
			throw new CladosMonadBinaryException(this, "Symm multiply fails reference match.", pM);
		else if (isGZero(this))
			return this;// obviously
		...
		MonadComplexF halfTwo = ((MonadComplexF) CladosGMonad.COMPLEXF.copyOf(this)).multiplyRight(pM);
		multiplyLeft(pM).add(halfTwo).scale(ComplexF.newONE(cM[0].getCardinal()).scale(CladosConstant.BY2_F));
		setGradeKey();
		return this;
	}
	 ...
	 */
	@Override
	public MonadComplexF normalize() throws CladosMonadException {
	 ...
	 */
	public void setCoeff(ComplexF[] ppC) throws CladosMonadException {
	 ...
	 */
	@Override
	public ComplexF sqMagnitude() throws CladosMonadException {
		try {
	 ...
	 * @return MonadComplexF
	 */
	public MonadComplexF subtract(MonadComplexF pM) throws FieldBinaryException, CladosMonadBinaryException {
		if (!isReferenceMatch(this, pM))
