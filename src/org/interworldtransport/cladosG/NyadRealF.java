...
public class NyadRealF extends NyadAbstract {
	...
	public static String toXMLFullString(NyadRealF pN, String indent) {
		...
		for (MonadRealF tSpot : pN.getMonadList())
			rB.append(MonadRealF.toXMLFullString(tSpot, indent+"\t\t"));
		rB.append(indent + "\t</MonadList>\n");
	...
	public static String toXMLString(NyadRealF pN, String indent) {
		...
		rB.append(indent + "\t<MonadList>\n");
		for (MonadRealF tSpot : pN.getMonadList())
			rB.append(MonadRealF.toXMLString(tSpot, indent + "\t\t"));
		rB.append(indent + "\t</MonadList>\n");
		rB.append(indent + "</Nyad>\n");
		return rB.toString();
	}
	 ...
	 *                              being used/copied.
	 */
	public NyadRealF(String pName, MonadRealF pM, boolean pCopy) throws CladosNyadException, CladosMonadException {
	 ...
	 *                              being copied.
	 */
	public NyadRealF appendMonadCopy(MonadRealF pM) throws CladosNyadException, CladosMonadException {
		// This method works if the foot of pM matches the foot of this nyad
		...
		// Add Monad to the ArrayList
		monadList.ensureCapacity(monadList.size() + 1);
		monadList.add((MonadRealF) CladosGMonad.REALF.copyOf(pM));
		resetAlgebraList(monadList);
		return this;
	}
	...
	public NyadRealF createMonad(String pName, String pAlgebra, String pFrame, String pSig, String pCard)
		...
		MonadRealF tM = (MonadRealF) CladosGMonad.REALF.createWithFoot(CladosFBuilder.DIVFIELD.createZERO(tCard),
	...
	public int findAlgebra(Algebra pAlg) {
		for (MonadRealF pM : getMonadList())
	 ...
	 * @return int
	 */
	public int findMonad(MonadRealF pIn) {
		for (MonadRealF pM : getMonadList())
			if (pIn == pM)
				return monadList.indexOf(pM);
		return -1;
