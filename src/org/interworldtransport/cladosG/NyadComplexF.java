...
public class NyadComplexF extends NyadAbstract {
	 ...
	 * @return String
	 */
	public static String toXMLFullString(NyadComplexF pN, String indent) {
		if (indent == null)
			indent = "\t";
		...
		for (MonadComplexF tSpot : pN.getMonadList())
			rB.append(MonadComplexF.toXMLFullString(tSpot, indent+"\t\t"));
		rB.append(indent + "\t</MonadList>\n");
	 ...
	 * @return String
	 */
	public static String toXMLString(NyadComplexF pN, String indent) {
		if (indent == null)
			indent = "\t";
		StringBuilder rB = new StringBuilder(indent+"<Nyad order=\"" + pN.getNyadOrder() + "\" ");
		rB.append("algorder=\"" + pN.getNyadAlgebraOrder() + "\" >\n");
		...
		rB.append(indent + "\t<MonadList>\n");
		for (MonadComplexF tSpot : pN.getMonadList())
			rB.append(MonadComplexF.toXMLString(tSpot, indent + "\t\t"));
		rB.append(indent + "\t</MonadList>\n");
		rB.append(indent + "</Nyad>\n");
		return rB.toString();
	}
	 ...
	 */
	public NyadComplexF(NyadComplexF pN) throws CladosNyadException, CladosMonadException {
	 ...
	 *                              being used/copied.
	 */
	public NyadComplexF(String pName, MonadComplexF pM, boolean pCopy)
	 ...
	 *                              monads in the nyad being copied.
	 */
	public NyadComplexF(String pName, NyadComplexF pN, boolean pCopy) throws CladosNyadException, CladosMonadException {
		setName(pName);
		setFoot(pN.getFoot());
	 ...
	 *                              being copied.
	 */
	public NyadComplexF appendMonadCopy(MonadComplexF pM) throws CladosNyadException, CladosMonadException {
		// This method works if the foot of pM matches the foot of this nyad
		...
		// Add Monad to the ArrayList
		monadList.ensureCapacity(monadList.size() + 1);
		monadList.add((MonadComplexF) CladosGMonad.COMPLEXF.copyOf(pM));
		resetAlgebraList(monadList);
		return this;
	}
	...
	public NyadComplexF createMonad(String pName, String pAlgebra, String pFrame, String pSig, String pCard)
		...
		MonadComplexF tM = (MonadComplexF) CladosGMonad.COMPLEXF
	...
	public int findAlgebra(Algebra pAlg) {
		for (MonadComplexF pM : getMonadList())
	 ...
	 * @return int
	 */
	public int findMonad(MonadComplexF pIn) {
		for (MonadComplexF pM : getMonadList())
			if (pIn == pM)
				return monadList.indexOf(pM);
		return -1;
