 ...
 * ------------------------------------------------------------------------ <br>
 * ---org.interworldtransport.cladosG.NyadComplexF<br>
 * -------------------------------------------------------------------- <p>
...
import static org.interworldtransport.cladosG.MonadComplexF.*;
 ...
 * physics.
 * <p>
 * NyadComplexF objects must be declared with at least one Monad that has at
 * least two generators of geometry. Each Monad on the list must have the same
...
public class NyadComplexF extends NyadAbstract {
	 ...
	 * @return int
	 */
	public static final int findAlgebra(NyadComplexF pN, Algebra pAlg) {
	 ...
	 * @return int
	 */
	public static final int findNextAlgebra(NyadComplexF pN, Algebra pAlg, int pStart) {
		if (pN.getMonadList().size() < pStart)
			return -1;
		for (int j = pStart; j < pN.getMonadList().size(); j++)
			if (pAlg.equals(pN.getMonadList(j).getAlgebra()))
				return j;
		return -1;
	}
	 ...
	 * @return boolean
	 */
	public static final int findFrame(NyadComplexF pN, String pFrame) {
		for (MonadComplexF pM : pN.getMonadList())
	 ...
	 * @return int
	 */
	public static final int findName(NyadComplexF pN, String pName) {
		for (MonadComplexF pM : pN.getMonadList())
	 ...
	 * 
	 * @param pN  NyadComplexF
	 * @param pIn MonadComplexF
	 * @return int
	 */
	public static final int findMonad(NyadComplexF pN, MonadComplexF pIn) {
		for (MonadComplexF pM : pN.getMonadList())
			if (pIn == pM)
				return pN.monadList.indexOf(pM);
		return -1;
	 ...
	 * @return boolean
	 */
	public static final boolean hasAlgebra(NyadComplexF pN, Algebra pAlg) {
	 ...
	 * @return boolean
	 */
	public static final boolean hasFrame(NyadComplexF pN, String pFrame) {
		for (MonadComplexF pM : pN.getMonadList())
			if (pFrame.equals(pM.getFrameName()))
				return true;
		return false;
	 ...
	 */
	public static final int howManyAtAlgebra(NyadComplexF pN, Algebra pAlg) {
	 ...
	 * @return boolean
	 */
	public static final boolean hasName(NyadComplexF pN, String pName) {
		for (MonadComplexF pM : pN.getMonadList())
			if (pName.equals(pM.getName()))
				return true;
		return false;
	 ...
	 * in memory
	 * 
	 * @param pTs NyadComplexF
	 * @param pN  NyadComplexF
	 * @return boolean
	 */
	public static final boolean isMEqual(NyadComplexF pTs, NyadComplexF pN) {
	 ...
	 * @return boolean
	 */
	public static final boolean isPScalarAt(NyadComplexF pN, Algebra pAlg) {
		boolean test = false; // Assume test fails
		if (pN.getMonadList().size() <= 0)
		...
		while (j < pN.getMonadList().size()) // loop through monads
		{
			...
			else if (!isGrade(pN.getMonadList(tSpot), maxGrade))
	 ...
	 * @return boolean
	 */
	public static final boolean isScalarAt(NyadComplexF pN, Algebra pAlg) {
		boolean test = false; // Assume test fails
		if (pN.getMonadList().size() <= 0)
			return false; // No monads? Fails.
		...
		while (j < pN.getMonadList().size()) // loop through monads
		{
			...
			if (tSpot < 0)
				break; // none found? break out of loop
			else if (!isGrade(pN.getMonadList(tSpot), 0))
	 ...
	 * the Nyads must match except for the NyadComplexF names. The monads within the
	 * NyadComplexF must also be reference matches for pairs from the same algebra.
	 ...
	 * @return boolean
	 */
	public static final boolean isStrongReferenceMatch(NyadComplexF pTs, NyadComplexF pN) {
	 ...
	 * Nyads must match except for the NyadComplexF names and orders. The monads
	 * within the NyadComplexF must also be reference matches for pairs from the
	 * same algebra. It is NOT required that monads from this nyad have counterparts
	 * in the other nyad, so the passed NyadComplexFs may have a different order
	 * than this one. Unpaired monads are counted as matches against scalars from
	 * the field. Only monads sharing the same algebra name need to be checked
	 * against each other for reference matches. For those in the same algebra, we
	 * make use of the isRefereceMatch method and compare the two.
	 * 
	 * @param pTs NyadComplexF
	 * @param pN  NyadComplexF
	 * @return boolean
	 */
	public static final boolean isWeakReferenceMatch(NyadComplexF pTs, NyadComplexF pN) {
	 ...
	 * 
	 * @param pN NyadComplexF This is the nyad to be converted to XML.
	 * 
	 * @return String
	 */
	 ...
	 * copied in detail. This contructor is used most often to get around operations
	 * that alter one of the nyads when the developer does not wish it to be
	 * altered.
	 * 
	 * @param pN NyadComplexF
	 * @throws CladosNyadException This exception is thrown when the offered Nyad is
	 *                             malformed. Make no assumptions!
	 */
	 ...
	 *                             malformed. Make no assumptions!
	 */
	public NyadComplexF(String pName, MonadComplexF pM, boolean pCopy) throws CladosNyadException {
		setName(pName);
	 ...
	 *                             malformed. Make no assumptions!
	 */
	public NyadComplexF(String pName, NyadComplexF pN, boolean pCopy) throws CladosNyadException {
		setName(pName);
		setFoot(pN.getFoot());
	...
	/**
	 * Dyadic antisymmetric compression: 1/2 (left right - right left) Monads are
	 * placed in the same algebra and antisymmetrically multiplied to each other. A
	 * reference match test must pass for both after the algebra names have been
	 * changed.
	 * 
	 * @param pInto AlgebraComplexF
	 * @param pFrom AlgebraComplexF
	 * @throws CladosNyadException This exception is thrown when the monads to be
	 *                             compressed fail a field match test or a reference
	 *                             match test used in multiplication.
	 * 
	 * @return NyadComplexF
	 */
	/*
	 * public NyadComplexF antisymmCompress(AlgebraComplexF pInto, AlgebraComplexF
	 * pFrom) throws CladosNyadException { // The strings refer to particular
	 * algebras. Find them in the AlgebraKey // to know which two monads are being
	 * compress. Once that is done, do // the operation. int tempInto =
	 * findAlgebra(this, pInto); int tempFrom = findAlgebra(this, pFrom);
	 * 
	 * try { antisymmCompress(tempInto, tempFrom); return this; } catch
	 * (FieldBinaryException e) { throw new CladosNyadException(this,
	 * "Monad cardinals must match for compression."); } catch
	 * (CladosMonadBinaryException e) { throw new CladosNyadException(this,
	 ...
	 */
	public void antisymmCompress(int pInto, int pFrom) throws FieldBinaryException, CladosMonadException {
	 ...
	 * @return NyadComplexF
	 */
	public NyadComplexF appendMonadCopy(MonadComplexF pM) throws CladosNyadException {
		// This method works if the foot of pM matches the foot of this nyad
	...
	public NyadComplexF createMonad(String pName, String pAlgebra, String pFrame, String pSig, String pCard)
		...
		MonadComplexF tM;
		if (pCard == null)
			tM = new MonadComplexF(pName, pAlgebra, pFrame, getFoot(), pSig,
					(ComplexF) CladosField.COMPLEXF.createZERO(getFoot().getCardinal(0)));
		else
			tM = new MonadComplexF(pName, pAlgebra, pFrame, getFoot(), pSig,
					(ComplexF) CladosField.COMPLEXF.createZERO(pCard));
		appendMonad(tM);
		return this;
	}
	 ...
	 * @return NyadComplexF
	 */
	// public NyadComplexF scale(AlgebraComplexF pk, ComplexF pMag) throws
	// FieldBinaryException
	// {
	 ...
	 * @param pFrom AlgebraComplexF
	 * 
	 * @throws CladosNyadException This exception is thrown when the monads being
	 *                             compressed fail a field match or reference match
	 *                             test used in multiplication.
	 * @return NyadComplexF
	 */
	/*
	 * public NyadComplexF symmCompress(AlgebraComplexF pInto, AlgebraComplexF
	 * pFrom) throws CladosNyadException { // The strings refer to particular
	 * algebras. Find them in the AlgebraKey // to know which two monads are being
	 * compress. Once that is done, do // the operation. int tempInto =
	 * findAlgebra(this, pInto); int tempFrom = findAlgebra(this, pFrom);
	 * 
	 * try { symmCompress(tempInto, tempFrom); return this; } catch
	 * (FieldBinaryException e) { throw new CladosNyadException(this,
	 * "Monad cardinals must match for compression."); } catch
	 * (CladosMonadBinaryException e) { throw new CladosNyadException(this,
	...
	private void resetAlgebraList() {
		...
		for (MonadComplexF point : monadList)
		...
		// Comparator<Algebra> comp = (Algebra a, Algebra b) -> {return
		// b.compareTo(a);};
	 ...
	 * @throws CladosMonadException
	 */
	public void symmCompress(int pInto, int pFrom) throws FieldBinaryException, CladosMonadException {
		MonadComplexF tempLeft = monadList.get(pInto);
		MonadComplexF tempRight = monadList.get(pFrom);
